/**
 * Scene4LeaderboardUI.tsx
 *
 * Leaderboard page for Scene 4 — identical visual to Scene3LeaderboardUI
 * but starting with content already visible (camera inherits Scene 3's end
 * state, so we skip the stagger-from-hidden and open at full opacity).
 *
 * The "View Levels" tap target is the back-chevron area (top-left, ~x=30, y=36)
 * which the Scene 4 orchestrator taps to navigate to the Levels page.
 */
...
const USERS = [
  ...
  { rank: 5, username: 'eg',             points: 10, photo: null,             color: '#22c55e', initials: 'E' as const },
...
const podiumVariants = {
  ...
  visible:  { opacity: 1, scale: 1, transition: { duration: 0.45, ease: [0.16, 1, 0.3, 1] as [number, number, number, number] } },
};

function Avatar({ user, size, borderColor, borderWidth = 2 }: {
  ...
  return (
    <div style={{
      ...
      {user.photo ? (
        ...
        <div style={{
          width: '100%', height: '100%', background: user.color,
...
export interface Scene4LeaderboardUIProps {
  revealControls?: AnimationControls;
}

export function Scene4LeaderboardUI({ revealControls }: Scene4LeaderboardUIProps) {
  const top3      = USERS.slice(0, 3);
  const listUsers = USERS.slice(3, 8);
  const currentUser = USERS[8];

  return (
    <div id="s4-leaderboard" style={{
      width: '390px', height: '100%', background: 'white',
      ...
      <div style={{
        ...
        {/* "View Levels" nav — tap target for Phase 3 */}
        <span id="s4-lb-view-levels" style={{
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {listUsers.map(user => (
            <motion.div key={user.rank} variants={itemVariants} style={{
              display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      ...
      <div style={{ padding: '12px 16px', background: 'white', borderTop: '1px solid #f9fafb' }}>
        <div style={{
          background: 'linear-gradient(90deg, #FF6B00, #FF9A3C)', borderRadius: 9999,
          padding: '4px 4px 4px 16px', display: 'flex', alignItems: 'center',
          justifyContent: 'space-between', boxShadow: '0 4px 12px rgba(255,107,0,0.25)',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
